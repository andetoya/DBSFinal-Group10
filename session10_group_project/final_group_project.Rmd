...
```{r}
#By type of merchant
#Create a table by merchant type with the number of value, proportion of fraudulent and legimitate
fraud_bymerch <- card_fraud %>%
   group_by(category) %>%
...
#By state
#Create a table by state with the number of trx, value, proportion of fraudulent and legimitate
fraud_bystate <- card_fraud %>%
   group_by(state) %>%
   summarize(total_legitimate_amt = sum(amt[is_fraud == 0]),
             total_fraudulent_amt = sum(amt[is_fraud == 1]),
             total_amt = sum(amt),
             fraudulent_percentage = (total_fraudulent_amt / total_amt),
             total_legitimate = sum(is_fraud == 0),
             total_fraudulent = sum(is_fraud == 1))


# Print the summary table
print(fraud_bystate)

#Create a bar chart by state in descent order
ggplot(fraud_bystate, aes(x = reorder(state, -fraudulent_percentage), y = fraudulent_percentage)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(x = "State", y = "% of Total Fraudulent Transactions per State", title = "Fraudulent Transactions by State")

#By city population
#First create object with percentages by city and include city_pop
fraud_bycity <- card_fraud %>%
   group_by(city) %>%
   summarize(total_legitimate_amt = sum(amt[is_fraud == 0]),
             total_fraudulent_amt = sum(amt[is_fraud == 1]),
             total_amt = sum(amt),
...
# Scatter plot by city population with trend line
ggplot(fraud_bycity, aes(x = city_pop, y = fraudulent_percentage)) +
  geom_point(shape = 16, color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(x = "City Population", y = "% of Fraudulent Transactions", title = "Fraudulent Transactions by City Population") + scale_x_continuous(labels = comma)  
```
...
#Clearly Job has too many unique values that would not help during the analysis. We would not use job as a category variable.
...
#Now i am going to select some key variables: is_fraud, amt, category, year, hour, wday, age, distance_km, month_name
my_card_fraud <- my_card_fraud %>% 
  select(is_fraud, amt, category, year, hour, wday, age, distance_km, month_name)
```
...
cost <- cost %>%
  ...
  false_negative= ifelse(pred == 0 & is_fraud == 1, 
                         amt, 0),

  
  
  # false positives-- we thought they were fraud, but they were not
  false_positive = ifelse(pred == 1 & is_fraud ==0, amt, 0),

  
    
  # true positives-- we thought they were fraud, and they were 
 true_positive = ifelse(pred == 1 & is_fraud ==1, amt, 0),

  
  # true negatives-- we thought they were ok, and they were 
  true_negative = ifelse(pred == 0 & is_fraud ==0, amt, 0)
...
#With this results, the model will reduce the cost from 2M (false_naive) to 500k (false_negative)
```
